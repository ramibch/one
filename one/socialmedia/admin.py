...
from .models import LinkedinAuth, LinkedinChannel
...
class LinkedinAuthAdmin(OneModelAdmin):
    ...
    readonly_fields = ("state", "code", "expires_at", "refresh_token_expires_at", "scope")
    actions = ("request_code", )

    @admin.action(description="🔑 Request code")
    def request_code(modeladmin, request, queryset):
        if queryset.count() > 1:
            messages.error(request, _("Not possible to request for multiple channels at the same time."))
            return
        obj = queryset.first()
