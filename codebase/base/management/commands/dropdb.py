...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if input() == f"Drop {db_name}":
            ...
            self.stdout.write(self.style.SUCCESS(f"Postgres database '{db_name}' is dropped."))
        else:
            self.stdout.write(self.style.WARNING(f"Confirmation declied. Not dropping '{db_name}'."))
