...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--delete-example",
            action="store_true",
            help="Remove previously the site example.com",
        )
        parser.add_argument(
            "--delete-all", action="store_true", help="Remove previously all the sites"
        )
    ...
    def handle(self, *args, **options):
        ...
        for env in options["environments"]:
            ...
            for site_name, site_domain in env_sites:
                extsite, created = ExtendedSite.objects.get_or_create(
        ...
        if len(created_sites) > 0:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"Sites for '{env}' successfully created:\n{created_sites_as_str}"
                )
            )
        else:
            self.stdout.write(
                self.style.WARNING(f"Sites for '{env}' are already created")
            )
